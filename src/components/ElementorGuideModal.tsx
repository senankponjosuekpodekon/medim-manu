...
export const ElementorGuideModal: React.FC<ElementorGuideModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-card border border-[#C5A059]/40 rounded-3xl p-6 sm:p-8 max-w-3xl w-full shadow-2xl relative space-y-6 max-h-[90vh] overflow-y-auto">
        
        <button
          onClick={onClose}
          className="absolute top-5 right-5 text-muted hover:text-main p-2 rounded-full bg-slate-800/60"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-2xl bg-icon border border-[#C5A059]/40 text-[#DFB86A]">
        ...
        {/* Tab Buttons */}
        <div className="flex flex-wrap gap-2 border-b border-soft pb-3">
          <button
            onClick={() => setActiveTab('elementor')}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
              activeTab === 'elementor' ? 'bg-[#C5A059] text-[#08120F]' : 'bg-base text-soft'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
              activeTab === 'seo' ? 'bg-[#C5A059] text-[#08120F]' : 'bg-base text-soft'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
              activeTab === 'palette' ? 'bg-[#C5A059] text-[#08120F]' : 'bg-base text-soft'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
              activeTab === 'jsonld' ? 'bg-[#C5A059] text-[#08120F]' : 'bg-base text-soft'
            }`}
          >
        ...
        {activeTab === 'elementor' && (
          <div className="space-y-4 text-xs text-soft leading-relaxed">
        ...
        {activeTab === 'seo' && (
          <div className="space-y-4 text-xs text-soft leading-relaxed">
            <div className="p-4 bg-base rounded-xl border border-soft space-y-3">
              ...
              <p className="text-main font-mono bg-[#050B09] p-2 rounded">{ELEMENTOR_SEO_MAP.seo.metaTitle}</p>
              ...
              <p className="text-main font-mono bg-[#050B09] p-2 rounded">{ELEMENTOR_SEO_MAP.seo.metaDescription}</p>
              ...
              <p className="text-main font-mono bg-[#050B09] p-2 rounded">{ELEMENTOR_SEO_MAP.seo.h1Text}</p>
            </div>
          </div>
        )}
        ...
        {/* Tab 3: Colors & Fonts */}
        {activeTab === 'palette' && (
          <div className="space-y-4 text-xs text-soft">
            <h4 className="font-serif-luxury text-lg font-bold text-main">
              Palette de Couleurs Hexadécimales :
            </h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {ELEMENTOR_SEO_MAP.colorPaletteHex.map((c) => (
                <div key={c.hex} className="p-3 rounded-xl bg-base border border-soft space-y-2">
                  <div className="w-full h-8 rounded-lg" style={{ backgroundColor: c.hex }} />
                  <div>
                    <p className="font-bold text-main text-[11px]">{c.name}</p>
        ...
        {activeTab === 'jsonld' && (
          <div className="space-y-3 text-xs text-soft">
            ...
            <pre className="p-4 bg-[#050B09] rounded-xl border border-soft font-mono text-[11px] text-amber-200 overflow-x-auto max-h-64">
              {jsonLdSchema}
            </pre>
          </div>
        )}
